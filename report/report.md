...
FFD文件的每个区域以双##开头，##内部表明从下一行开始的数据含义，如dimension，offsets等。从##的下一行开始到下一个##号或文件截止为该##号所描述的数据内容。控制点的位移由一个三维的数组表示，具体的：设三个维度为$[x,y,z]$,不同的x维度之间以一个空行隔开，同一x维之间不同的y维由
一个换行符隔开，同一xy维之间不同的z维由一个tab隔开，每个xyz维存储一个三维的控制点位移，每个维度之间由空格隔开。
...
2. ##### 图像处理：
   ...
   首先由读入的OBJ文件计算初始的控制点位置，其次若加载FFD文件则对每一个控制点更新位移。
