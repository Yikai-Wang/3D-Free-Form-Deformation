...
-  控制点交互方面我们受到了Github仓库[3D-FFD-in-VTK]((https://github.com/Anthony-Xu/3D-FFD-in-VTK))的启发，在此基础上进行了一些精简。
...
2. ##### 文件导入和存储：
   ...
   **OBJ文件：**我们使用VTK自带的`vtkOBJReader`读取.obj文件，使用`GetOutput()`获得模型的`PolyData`数据集，但是这样存在的问题是，我们无法获得模型每个点和面对应的颜色的RGB值，因此我们再手写了一个读入函数，重新遍历一遍.obj文件，获取$Point\rightarrow RGB$的对应关系。.obj文件读取分为两个部分：
   ...
   -  通过VTK的`OBJReader`读取.obj文件画图，通过自写`reader`函数存储$Point \rightarrow RGB$。
   -  按照.obj文件的规则读取每个点的坐标，存储到内存中，为FFD算法的计算做准备。

   ​

   **FFD文件：** 我们手写了一个读入.ffd文件函数，获取了控制点阵中每个点的(x,y,z)坐标值，以及位置偏移，再通过复用为Modify-Set XYZ中设计的控制点回调函数`SphereQt()`，将.ffd读入的控制点逐个进行移动。
   ...
   **PNG文件：**我们读入人脸的二维照片，再利用论文 [Joint 3D Face Reconstruction and Dense Alignment with Position Map Regression Network](https://arxiv.org/abs/1803.07835) 提出的PRNet对照片中的人脸进行3D重建。模型直接使用已训练好的模型[PRNet](https://github.com/YadiraF/PRNet)，导入带有颜色信息的.obj文件，并通过上文提到的方式读入到VTK中:
   ...
   ![alignment](pics/PRNet.jpg)

   ​

   **OBJ、FFD文件存储：**由于VTK中并没有`vtkOBJWriter`等函数，我们根据.ffd和.obj文件格式写入新文件，**并考虑到是否有纹理两种情况，**按照上文所述文件格式逐行写入。
...
3. ##### 模型处理：
   ...
   | ![color](./pics/colors.jpg) | ![color](./pics/resizes.jpg) |
   | :-------------------------: | :--------------------------: |
   |             上色              |             减采样              |
...
4. ##### 其他功能：
   ...
   | ![dots](./pics/dotss.jpg) | ![dots](./pics/resets.jpg) |
   | :-----------------------: | :------------------------: |
   |         设置控制点阵数目          |             重置             |
...
-  **开发语言：**Python>=3.5
-  **软件包：**numpy>=1.13.0, tensorFlow >= 1.4, vtk=8.1.0, PyQt=5.9.6 
...
-  **使用环境：**Windows 10
-  **文件名称**：15300180076_15307130424_15307130175_可执行文件.zip
-  **使用方式：** 
   1. 解压zip文件，点击文件内FFD.exe执行文件。
   2. 使用方式如本文第三节 **可视化界面GUI**所述，其中zxh-ape.obj文件是默认加载模型，请勿更改名称，其他的.obj和.ffd文件均提供作为测试所用，也可以生成使用更多.obj或.ffd文件。
-  **补充说明：**
   1. 我们原计划用MacOS实现所有功能，但由于VTK与Mac的PyQt交互会出现鼠标偏移，我们长时间debug未果，因此换成Windows平台。
   2. 在zip文件夹内我们放置了.dll动态链接文件，希望解决其他电脑打开exe不兼容的问题。我们的正式解决办法是添加环境变量，具体步骤如该[链接](https://stackoverflow.com/questions/48007100/error-could-not-find-or-load-the-qt-platform-plugin-windows-pyqt-pyinstal)所言。如果.dll在电脑上不起作用，可以尝试链接中的解决办法。
   3. 由于换成Windows平台后，我们的人脸3D重建接口所依赖的TensorFlow库无法打包成exe文件部分，因此Load .PNG改为通过python命令行实现，具体方式可以联系我们，或者参考这个[链接](https://github.com/YadiraF/PRNet)。
   4. 经测试，程序能够完成报告上述所有功能，但偶尔出现闪退现象，如反复加载ffd文件和obj文件、用不协调的ffd文件移动当下控制点阵等。
   5. 精细人像在减采样之前大约要4秒左右才能进行形变响应，具体时间取决于机器性能，请耐心等待，Have fun!
...
-  **何占魁：** GUI实现与打包，参与模型形变实现，算法优化，报告撰写
