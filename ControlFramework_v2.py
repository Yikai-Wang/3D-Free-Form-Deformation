import vtk
from FFD_v2 import obj_reader, FFD
import numpy as np
...
def sphereCallback(obj, event):
    ...
    t1 = time()
    for (u,v,w) in ffd.changed.keys():
        for i in range(-2,2):
            for j in range(-2,2):
                for k in range(-2,2):
                    if 0<=u+i<ffd.cp_num_x and 0<=v+j<ffd.cp_num_y and 0<=w+k<ffd.cp_num_z:
                        for (id_index,x,y,z) in ffd.object_points[(u+i,v+j,w+k)]:
                            tmp = ffd.T_local([x,y,z])
