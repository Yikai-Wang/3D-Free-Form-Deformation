...
class VtkModel(object):
    def __init__(self, ren=None, iren=None, filename="zxh-ape.obj", RESIZE = 1, COLOR = True, RADISU = 5, xl = 4, yl = 4, zl = 4):
        # 参数初始化
        self.ren = ren
        self.iren = iren
    ...
    def drawFace(self):
        ...
        # 如果需要着色的话
        if self.COLOR :
            self.data = color_on_points(self.data, read_color_from_ffd(self.filename))
        # 如果需要压缩图像的话
        if self.RESIZE != 1:
            self.data = resize_poly_data(self.data, self.RESIZE)
